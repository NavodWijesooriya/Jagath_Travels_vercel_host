...
const TranslationWidget: React.FC = () => {
  ...
  const [scriptUrl, setScriptUrl] = useState<string>(`/js/gt.min.js?v=${pathname}`);
  ...
  useEffect(() => {
    ...
    setScriptUrl(`/js/gt.min.js?v=${pathname}`);
  }, [scriptUrl]);

  return (
    <div id="gt-mordadam-43217984" className={Styles.translate}>
      <Script
        src={scriptUrl} // Make sure this path is correct and the file is available in the public directory
